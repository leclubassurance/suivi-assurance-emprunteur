...
export async function createConseillerPortalSession(apporteurId: string, res: Response): Promise<void> {
...
export async function resolveConseillerPortalSession(req: Request): Promise<Apporteur | null> {
  const cookie = parseCookies(req)[CONSEILLER_SESSION_COOKIE];
